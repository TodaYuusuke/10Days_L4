...
// 弾幕攻撃stateクラス
class CircleBarrageState : public IEnemyState {
public:
	CircleBarrageState() = delete;
	CircleBarrageState(const BaseEnemyData* data, NormalBossStateManager* sManager, std::array<EnemyBullet, NormalBossData::kNormalBossBulletMax>* bullets);
	~CircleBarrageState() = default;
...
private:
	// 弾の発射
	void FireBullet(const Vector2& pos);
	// 弾の座標更新
	void BulletUpdate();
	...
	std::vector<LWP::Math::Vector2> dirs_;
	bool isBullet_ = false; // 弾が発射
	uint8_t divisionCount_ = 0u;
};
